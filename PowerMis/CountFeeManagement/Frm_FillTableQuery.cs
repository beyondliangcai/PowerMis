...
{
    public partial class Frm_FillTableQuery : Form
    {
        ...
        private void Frm_FillTableQuery_Load(object sender, EventArgs e)
        {
        ...
        private void cbxBookNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                string strSQL = "SELECT CustomerNo,CustomerName,CustomerPosition FROM CustomerInfo WHERE Left(CustomerNo,5) = '" + cbxBookNo.Text.Trim() + "' order by CustomerPosition";
        ...
        private double calcPowerPrice(string customerNo, int countPowerValue) //计算电费
        {
            ...
            try
            {
                ...
                if (negativedataTable.Rows.Count > 0)
                {//如果今年存在负数发票
        ...
        public void calcPowerFee()
        {
            ...
            //int countPowerValue = int.Parse((powerValue * AmmeterMulti + lineLoss + transLoss).ToString());
            //低保户处理
            /*
            ...
            */
            ...
            double PowerFee = calcPowerPrice(txtCustomerNo.Text, countPowerValue); //综合电价
            ...
            double totalPrice = PowerFee + basePrice;        //总电费
            if (DiscountRate > 0)
            {
        ...
        private void updateFee()
        {
            ...
            {
                MessageBox.Show("该客户本月数据尚未录入！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                //saveFlag = false;
                return;
            }
            ...
            try
            {
                strSql = "UPDATE AmmeterValue SET AmmeterValue=" + txtThisMonthValue.Text + ",Inputtime=GETDATE(),Inputman='" + Constant.LoginUser.UserName + "'"
                        + " WHERE CustomerNo = '" + txtCustomerNo.Text + "' And CopyValueDate = '" + time +"'";
                SQLUtl.ExecuteSql(strSql);

            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message.ToString()+"456465");
                //saveFlag = false;
                return;
            }
            //int powerValue = int.Parse(txtThisMonthValue.Text) - int.Parse(txtLastMonthValue.Text);
            float powerValue = float.Parse(txtPowerValue.Text.ToString());
            ...
            try
            {
                ...
                ds = SQLUtl.Query(strSql);
                if (ds.Tables[0].Rows.Count != 0 && ds.Tables[0].Rows[0]["AdvanceDeposit"].ToString() != null)
                {
                    lastBalance = float.Parse(ds.Tables[0].Rows[0]["AdvanceDeposit"].ToString());
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                strSql = "UPDATE CountFee SET StartCode=@startCode, EndCode=@endCode, MonthCopyAmount=@monthCopyAmount, CountFeeAmount=@countFeeAmount,"
                        + "CountFeeMoney=@countFeeMoney, AbnormityAmount=@abnormityAmount, LineLose=@lineLose,"
                        + "TransformerLose=@transformerLose, CityAnnex=@cityAnnex, CountryAnnex=@countryAnnex, BaseFeeMoney=@baseFeeMoney, TotalMoney=@totalMoney,"
                        + "FactRec=@factRec,AdvanceDeposit=@advanceDeposit,AccountRec=@accountRec,"
                        + "InputTime=GETDATE(), InputMan=@inputMan, LastMonthStart=@lastMonthStart,CustomerName=@customerName,CustomerAddress=@customerAddress,AmmeterMulti=@ammeterMulti, "
                        + "LineNum=@lineNum,AreaNo=@areaNo,TransformerNo=@transformerNo,LineLoseRate=@lineLoseRate,DiscountRate=@discountReate,InvoiceType=@invoiceType,ElectriCharacterName=@electricharacter,Especialflag=@especialFlag,"
                        ...
                        + " WHERE CustomerNo = '" + txtCustomerNo.Text + "' And CountFeeDate = '" + time + "'and NegativeInvoiceFlag=0";
                SqlParameter[] sqlParams = { 
                    new SqlParameter("@startCode", txtLastMonthValue.Text),
                    new SqlParameter("@endCode", txtThisMonthValue.Text),
                    new SqlParameter("@monthCopyAmount", MonthCopyAmount),
                    new SqlParameter("@countFeeAmount", txtCountPowerValue.Text),
                    new SqlParameter("@countFeeMoney", CountFeeMoney),
                    new SqlParameter("@abnormityAmount", AbnormityAmount),
                    new SqlParameter("@lineLose", LineLoss),
                    new SqlParameter("@transformerLose", txtChangeLoss.Text),
                    new SqlParameter("@cityAnnex", txtCityPrice.Text),
                    new SqlParameter("@countryAnnex", txtRuralPrice.Text),
                    new SqlParameter("@baseFeeMoney", txtBasePrice.Text),
                    new SqlParameter("@totalMoney", txtTotalPrice.Text),
                    new SqlParameter("@factRec", actFee),
                    new SqlParameter("@advanceDeposit", thisBalance),
                    new SqlParameter("@accountRec", preFee),
                    ...
                    new SqlParameter("@customerAddress", customer.getCustomerAddress()),
                    new SqlParameter("@ammeterMulti", AmmeterMulti),
                    new SqlParameter("@lineNum", customer.getLine()),
                    new SqlParameter("@areaNo", customer.getArea()),
                    new SqlParameter("@transformerNo", TransformerNo),
                    new SqlParameter("@lineLoseRate", LineLoseRate),
                    new SqlParameter("@discountReate", DiscountRate),
                    new SqlParameter("@invoiceType", customer.getInvoiceType()),
                    new SqlParameter("@electricharacter", customer.getElectriCharacterName()),
                    new SqlParameter("@especialFlag", customer.getEspecialFlag()),
                    //new SqlParameter("@transFormerNo", customer.getT),
        ...
        {
            txtLastMonthValue.Enabled = true;
            txtThisMonthValue.Enabled = true;
            txtAbnormityAmount.Enabled = true;
        }
        ...
        {
            txtThisMonthValue.Enabled = false;
            txtAbnormityAmount.Enabled = false;
            if (txtCustomerNoKey.Text != "")
            {
                init_SelectedCustomerInfo(txtCustomerNoKey.Text);
            }
